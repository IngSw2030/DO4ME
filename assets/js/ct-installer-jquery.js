...
jQuery(document).ready(function(){
	...
	jQuery(document).on('click','.database_check_con',function(){
		...
		if (!jQuery('#d4mdb_form').valid()) {
			...
			jQuery.ajax({
				...
				url : obj_installer.ajax_url+'installer_ajax.php',
				success : function(res){
					jQuery('.text-center').html(res);
					jQuery('.connection_error').show();
					if(res.indexOf("product purchase code verified now!") > -1){
	...
	jQuery(document).on('click','.admin_credential_next',function(){
		jQuery('#d4madmin_detail_form').validate();

		jQuery("#d4madmin_email").rules("add",
		{
			required: true,
			messages: {required: "Please Set Your Username"}
		});
		jQuery("#d4madmin_password").rules("add",
		{
			required: true,
			minlength : 8,
			maxlength : 15,
		...
		if (!jQuery('#d4madmin_detail_form').valid()) {
			return false;
		}
		jQuery.ajax({
			type : 'post',
