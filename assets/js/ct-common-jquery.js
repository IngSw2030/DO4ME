  ...
  jQuery.ajax({
    ...
    success : function(res){
      ...
      if(methods_data.status == "single"){
        ...
        jQuery.ajax({
          type : "post",
          data : { "service_id" : id, "staff_select_according_service" : 1 },
          url : ajax_url+"front_ajax.php",
          success : function(res){
    ...
    success : function(res){
      ...
      jQuery(".select-bedrooms").each( function(){
        if(jQuery(this).attr("data-un_id") == unit_id && jQuery(this).attr("data-mnamee") == m_name){
          ...
          jQuery("#ct_selected_"+unit_id+" .d4m-count").html(dd_default_title);
        }
      });
      ...
      jQuery("#postalcodefrm").validate({
		  ...
		  messages: {
			ct_postal_code:{
			  ...
			  digits: "Solo se permiten códigos postales de bogotá"
			},
		  }
		});
		...
		if (!jQuery("input[name='service-radio']").is("checked")) {
			jQuery(".custom_service_error").first().html("Por favor seleccione un servicio");
			service = 0;
		}else{
		...
		if(jQuery(".custom_service_error").css('display') == 'none'){
			...
			if( jQuery('.cart-items-main').find('ul').find("li").length == 0){
				...
				jQuery(".custom_item_error").first().html("Por favor seleccione un servicio");
				service_item = 0;
			}else{
				service_item = 1;
			}
		...
		//alert(jQuery('.ct-total-amount').text());
		//alert(postalcode);
		//alert(service);
		//alert(service_item);
