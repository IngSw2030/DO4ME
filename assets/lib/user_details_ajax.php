...
$objadminprofile = new do4me_adminprofile();$objadminprofile->conn = $conn;
...
$date_format=$setting->get_option('d4mdate_picker_date_format');
$time_format = $setting->get_option('d4mtime_format');
$getmaximumbooking = $setting->get_option('d4mmax_advance_booking_time');
...
$company_email = $setting->get_option('d4mcompany_email'); 
$company_address = $setting->get_option('d4mcompany_address');
...
if(isset($_POST['getmytimeslots'])){
    ...
    $seled4mtime=date('Y-m-d',strtotime($_POST['selected_dates']));
    ...
    <select class="selectpicker mydatepicker_appointment form-control" id="myuser_reschedule_time" data-size="10" style="">
        ...
        if($time_schedule['off_day']!=true && isset($time_schedule['slots']) && sizeof((array)$time_schedule['slots'])>0 && $allbreak_counter != sizeof((array)$time_schedule['slots']) && $allofftime_counter != sizeof((array)$time_schedule['slots'])){
            foreach($time_schedule['slots']  as $slot) {
								$curreslotstr = strtotime(date(date('Y-m-d H:i:s',strtotime($seled4mtime.' '.$slot)),$currDateTime_withTZ));
								$gccheck = 'N';
                ...
                if( in_array($complete_time_slot,$time_schedule['booked']) && ($setting->get_option('d4mallow_multiple_booking_for_same_timeslot_status')!='Y') ) { ?>
                    ...
                    if($setting->get_option('d4mhide_faded_already_booked_time_slots')=="on"){
                        ?>
                        <option value="<?php echo date("H:i",strtotime($slot));?>" class="time-slot br-2 d4m-booked" >
                            <?php  
                ...
                <?php  
                    ...
                    ?>
                    <option value="<?php echo date("H:i",strtotime($slot));?>" class="time-slot br-2 <?php   if(in_array($complete_time_slot,$time_schedule['booked'])){ echo' d4m-booked';}else{ echo ' time_slotss'; }?>" <?php   if(in_array($complete_time_slot,$time_schedule['booked'])){echo ''; }else{ echo 'data-slot_date_to_display="'.date($date_format,strtotime($_POST["selected_dates"])).'" data-slot_date="'.$_POST["selected_dates"].'" data-slot_time="'.$slot_time.'"'; } ?>><?php if($setting->get_option('d4mtime_format')==24){echo date("H:i",strtotime($slot));}else{echo str_replace($english_date_array,$selected_lang_label,date("h:i A",strtotime($slot)));}?></option>
                <?php  
                } $slot_counter++;
            }
...
if(isset($_POST['delete_gc_booking'])){
	...
	$pid = "";
	if($gc_hook->gc_purchase_status() == 'exist'){
		if($setting->get_option('d4mgc_status_configure') == 'Y' && $setting->get_option('d4mgc_status') == 'Y') {
        ...
        if($setting->get_option('d4msms_plivo_status')=="Y"){
            if($setting->get_option('d4msms_plivo_send_sms_to_client_status') == "Y"){
                ...
                if($template[4] == "E"){
                    ...
                    $params = array(
                        'src' => $setting->get_option('d4msms_plivo_sender_number'),
                        'dst' => $phone,
  ...
  $id=$order = $_POST['id'];
	...
	/* staff sms sending code */
        ...
        $replacearray = array($service_name, $booking_date , $business_logo, $business_logo_alt, $client_name,$methodname, $units, $addons,$client_email, $client_phone, $payment_status, $final_vc_status, $final_p_status, $client_notes, $client_status,$client_address,$price,$get_admin_name,$firstname,$lastname,'','',$admin_company_name,$booking_time,$client_city,$client_state,$client_zip,$company_city,$company_state,$company_zip,$company_country,$company_phone,$company_email,$company_address,$get_admin_name,stripslashes($get_staff_name),stripslashes($get_staff_email));
				...
				if($setting->get_option('d4mstaff_email_notification_status') == 'Y' && $clientemailtemplate[4]=='E' ){
					$client_email_body = str_replace($searcharray,$replacearray,$clienttemplate);
					if($setting->get_option('d4msmtp_hostname') != '' && $setting->get_option('d4memail_sender_name') != '' && $setting->get_option('d4memail_sender_address') != '' && $setting->get_option('d4msmtp_username') != '' && $setting->get_option('d4msmtp_password') != '' && $setting->get_option('d4msmtp_port') != ''){
							$mail->IsSMTP();
					}else{
...
if(isset($_POST['insert_crm_user_detail'])){
	...
	if($setting->get_option('d4mstripe_payment_form_status') == "on" && $setting->get_option('d4mstripe_create_plan') == "Y"){
		include(dirname(dirname(dirname(__FILE__))).'/assets/stripe/stripe.php');
		$secret_key = $setting->get_option('d4mstripe_secretkey');
		try{
