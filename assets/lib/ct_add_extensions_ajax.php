...
if (extension_loaded('zip')) {
	...
	if(isset($_POST['action']) && $_POST['action'] == "add_extension"){
		...
		$settings->set_option_check($_POST['purchase_option'],'Y');
		$settings->set_option_check($_POST['version_option'],$_POST['update_version']);
		if (( ($aV != '' || $aV == '') && $aV < $_POST['update_version'] && (!file_exists(dirname(dirname(dirname(__FILE__))).'/extension/'.$version_file_name.'.zip') || !is_dir(dirname(dirname(dirname(__FILE__))).'/extension/'.$_POST['extension']))) || ($aV == $_POST['update_version'] && (!file_exists(dirname(dirname(dirname(__FILE__))).'/extension/'.$version_file_name.'.zip') || !is_dir(dirname(dirname(dirname(__FILE__))).'/extension/'.$_POST['extension'])))){
			...
			if($updated){
				$d4mpayment_extensions = $settings->get_option('d4mpayment_extensions');
				$unserialize_d4mpayment_extensions = unserialize($d4mpayment_extensions);
				$keySearch = $_POST['extension'];
				$counts = 0;
				foreach ($unserialize_d4mpayment_extensions as $key => $item) {
					if ($key == $keySearch) {
						 $counts = $counts + 1;
					}
				}
				if($counts == 0){
					$new_payment_extensions = array(
						"method" => "direct",
						"include_path" => '/extension/'.$_POST['extension']."/".$_POST['extension'].".php",
						"option_name" => $_POST['purchase_option']
					);
					$unserialize_d4mpayment_extensions[$_POST['extension']] = $new_payment_extensions;
					$serialize_d4mpayment_extensions = serialize($unserialize_d4mpayment_extensions);
					$settings->set_option('d4mpayment_extensions',$serialize_d4mpayment_extensions);
				}
			}
		}
	}
		...
		if (( ($aV != '' || $aV == '') && $aV < $_POST['update_version'] && (!file_exists(dirname(dirname(dirname(__FILE__))).'/extension/'.$version_file_name.'.zip') || !is_dir(dirname(dirname(dirname(__FILE__))).'/extension/'.$_POST['extension']))) || ($aV == $_POST['update_version'] && (!file_exists(dirname(dirname(dirname(__FILE__))).'/extension/'.$version_file_name.'.zip') || !is_dir(dirname(dirname(dirname(__FILE__))).'/extension/'.$_POST['extension'])))){
			...
			if($updated){
				if($_POST['payment_option'] != ''){
					...
					if($counts==0){
						$unserialize_d4mpayment_extensions[$keySearch] = $unserialize_payment_option[$keySearch];
						$serialize_d4mpayment_extensions = serialize($unserialize_d4mpayment_extensions);
						$settings->set_option('d4mpayment_extensions',$serialize_d4mpayment_extensions);
						print_r($_POST['payment_add_option']);
