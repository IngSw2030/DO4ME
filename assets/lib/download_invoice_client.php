...
ob_start();	session_start();
	...
	$order=new do4me_order_client_info();
	$payments=new do4me_payments();	
	$general=new do4me_general();
	$smethod=new do4me_services_methods();
	$smunit=new do4me_services_methods_units();	
	$saddon=new do4me_services_addon();
...
include(dirname(dirname(dirname(__FILE__))).'/assets/lib/date_translate_array.php');
	...
	$dateformat=$setting->get_option('d4mdate_picker_date_format');
    $symbol_position=$setting->get_option('d4mcurrency_symbol_position');    
    $symbol=$setting->get_option('d4mcurrency_symbol');    
	...
	$business_name=$setting->get_option('d4mcompany_name');
	$business_email=$setting->get_option('d4mcompany_email');
	$business_address=$setting->get_option('d4mcompany_address');
	$business_city=$setting->get_option('d4mcompany_city');
	$business_state=$setting->get_option('d4mcompany_state');
	$business_zip=$setting->get_option('d4mcompany_zip_code');
	$business_country=$setting->get_option('d4mcompany_country');	
	$business_logo=$setting->get_option('d4mcompany_logo');
	...
	elseif($payinfo[2]=='Card Payment')
	{
		$payment_method = $label_language_values['card_payment']; 
	}	
	elseif($payinfo[2]=='Bank Transfer')
	{ 
		$payment_method = 'Bank Transfer';
	}	
	elseif($payinfo[2]=='Paypal')
	{ 
		$payment_method = 'Paypal';
	}	
	elseif($payinfo[2]=='Stripe-payment' || $payinfo[2]=='Card-payment' || $payinfo[2]=='2checkout-payment' || $payinfo[2]=='Stripe-Reccurance')
	{ 
	...
	if($book_info['method_id']!=='' || $book_info['method_id']!==null){
		...
		$service_price=$general->d4mprice_format_for_pdf($book_info['method_unit_qty_rate'],$symbol_position,$decimal);
	...
	foreach($all_addon_details as $book_add_info){
		...
		$addonprice=$general->d4mprice_format_for_pdf($book_add_info['addons_service_rate'],$symbol_position,$decimal);
	...
	$pdf->Text(155-$pdf->GetStringWidth($label_language_values['tax']),$addondetails_sttpoint+20,$label_language_values['tax']);
	 ...
	 $printamount=$general->d4mprice_format_for_pdf($payment_amount,$symbol_position,$decimal); 
	 $printtaxes=$general->d4mprice_format_for_pdf($payment_taxes,$symbol_position,$decimal);	  
	 $printdiscount='-'.$general->d4mprice_format_for_pdf($payment_discount,$symbol_position,$decimal);	   
	 $printfrequency='-'.$general->d4mprice_format_for_pdf($payinfo[12],$symbol_position,$decimal);	   
	 $printnetamount=$general->d4mprice_format_for_pdf($payment_net_amount,$symbol_position,$decimal);
