...
$objadmin->conn = $conn;
$objbooking = new do4me_booking();
$objbooking->conn = $conn;
$setting = new do4me_setting();
$setting->conn = $conn;
if($setting->get_option('d4msmtp_authetication') == 'true'){
	$mail_SMTPAuth = '1';
	if($setting->get_option('d4msmtp_hostname') == "smtp.gmail.com"){
		$mail_SMTPAuth = 'Yes';
	}
}else{
	$mail_SMTPAuth = '0';
...
$mail->Host = $setting->get_option('d4msmtp_hostname');
$mail->Username = $setting->get_option('d4msmtp_username');
$mail->Password = $setting->get_option('d4msmtp_password');
$mail->Port = $setting->get_option('d4msmtp_port');
$mail->SMTPSecure = $setting->get_option('d4msmtp_encryption');
$mail->SMTPAuth = $mail_SMTPAuth;
$mail->CharSet = "UTF-8";
$mail_a = new do4me_phpmailer();
$mail_a->Host = $setting->get_option('d4msmtp_hostname');
$mail_a->Username = $setting->get_option('d4msmtp_username');
$mail_a->Password = $setting->get_option('d4msmtp_password');
$mail_a->Port = $setting->get_option('d4msmtp_port');
$mail_a->SMTPSecure = $setting->get_option('d4msmtp_encryption');
$mail_a->SMTPAuth = $mail_SMTPAuth;
$mail_a->CharSet = "UTF-8";
$mail_s = new do4me_phpmailer();
$mail_s->Host = $setting->get_option('d4msmtp_hostname');
$mail_s->Username = $setting->get_option('d4msmtp_username');
$mail_s->Password = $setting->get_option('d4msmtp_password');
$mail_s->Port = $setting->get_option('d4msmtp_port');
$mail_s->SMTPSecure = $setting->get_option('d4msmtp_encryption');
$mail_s->SMTPAuth = $mail_SMTPAuth;
$mail_s->CharSet = "UTF-8";
/*NEXMO SMS GATEWAY VARIABLES*/
$nexmo_admin->d4mnexmo_api_key = $setting->get_option('d4mnexmo_api_key');
$nexmo_admin->d4mnexmo_api_secret = $setting->get_option('d4mnexmo_api_secret');
$nexmo_admin->d4mnexmo_from = $setting->get_option('d4mnexmo_from');
$nexmo_client->d4mnexmo_api_key = $setting->get_option('d4mnexmo_api_key');
$nexmo_client->d4mnexmo_api_secret = $setting->get_option('d4mnexmo_api_secret');
$nexmo_client->d4mnexmo_from = $setting->get_option('d4mnexmo_from');
$general=new do4me_general();
$general->conn=$conn;
$symbol_position=$setting->get_option('d4mcurrency_symbol_position');
$decimal=$setting->get_option('d4mprice_format_decimal_places');
$emailtemplate=new do4me_email_template();
$emailtemplate->conn=$conn;
$getcurrency_symbol_position=$setting->get_option('d4mcurrency_symbol_position');
$getdateformate = $setting->get_option('d4mdate_picker_date_format');
$time_format = $setting->get_option('d4mtime_format');
$booking = new do4me_booking();
$booking->conn = $conn;
$lang = $setting->get_option("d4mlanguage");
$label_language_values = array();
$language_label_arr = $setting->get_all_labelsbyid($lang);
/*SMS GATEWAY VARIABLES*/
$plivo_sender_number = $setting->get_option('d4msms_plivo_sender_number');
$twilio_sender_number = $setting->get_option('d4msms_twilio_sender_number');
/* textlocal gateway variables */
$textlocal_username =$setting->get_option('d4msms_textlocal_account_username');
$textlocal_hash_id = $setting->get_option('d4msms_textlocal_account_hash_id');
/*NEED VARIABLE FOR EMAIL*/
$company_city = $setting->get_option('d4mcompany_city'); $company_state = $setting->get_option('d4mcompany_state'); $company_zip = $setting->get_option('d4mcompany_zip_code'); $company_country = $setting->get_option('d4mcompany_country'); 
$company_phone = strlen($setting->get_option('d4mcompany_phone')) < 6 ? "" : $setting->get_option('d4mcompany_phone');
$company_email = $setting->get_option('d4mcompany_email');$company_address = $setting->get_option('d4mcompany_address'); 
/*CHECK FOR VC AND PARKING STATUS*/
$global_vc_status = $setting->get_option('d4mvc_status');
$global_p_status = $setting->get_option('d4mp_status');
$admin_phone_twilio = $setting->get_option('d4msms_twilio_admin_phone_number');
$admin_phone_plivo = $setting->get_option('d4msms_plivo_admin_phone_number');
...
$admin_email = $setting->get_option('d4madmin_optional_email');
/* set admin name */
/* set business logo and logo alt */
 if($setting->get_option('d4mcompany_logo') != null && $setting->get_option('d4mcompany_logo') != ""){
	$business_logo= SITE_URL.'assets/images/services/'.$setting->get_option('d4mcompany_logo');
	$business_logo_alt= $setting->get_option('d4mcompany_name');
}else{
	$business_logo= '';
	$business_logo_alt= $setting->get_option('d4mcompany_name');
}
...
if(isset($_POST['getgc_event_detail'])){
	$t_zone_value = $setting->get_option('d4mtimezone');
	...
	<div class="vertical-alignment-helper">
		<div class="modal-dialog modal-md vertical-align-center">
          ...
          <ul class="list-unstyled d4m-cal-booking-details mypopupul">
						<li class="gc_li">
          ...
          <div class="cta-col12 d4m-footer-popup-btn text-center">
						<div class="fln-mrat-dib ">
						<?php           
						if($currDateTime_withTZ <= $CalenderBooking["start"]){
						?>
							<span class="col-xs-6 np d4m-w-32 myconfirmclass">
								<a class="btn btn-link d4m-small-btn d4m-reschedual-calendar-appointment-cal" data-id="<?php    echo $_POST['orderid'];?>" data-duration="<?php    echo $total_duration;?>" title="<?php       echo $label_language_values['rescheduled']; ?>"><i class="fa fa-pencil-square-o fa-2x"></i><br><?php       echo $label_language_values['rescheduled']; ?></a>
							</span>
						<?php      } ?>
							<span class="col-xs-6 np d4m-w-32">
								<a data-id="<?php echo $_POST['orderid'];?>" id="d4m-delete-appointment-cal-popup" class="d4m-delete-appointment-cal-popup btn btn-link d4m-small-btn booking_deletess" rel="popover" data-placement='top' title="<?php echo $label_language_values['delete_this_appointment'];?>"><i class="fa fa-trash-o fa-2x"></i><br /> <?php  echo $label_language_values['delete'];?></a>
							</span>
								...
								<table class="form-horizontal" cellspacing="0">
									...
									<tr>
										<td>
											...
											<button id="d4m-close-del-appointment-cal-popup" class="btn btn-default btn-sm" href="javascript:void(0)"><?php echo $label_language_values['cancel'];?></button>
										</td>
									</tr>
									</tbody>
								</table>
                ...
                <div class="modal-body mb-20">
                    <ul class="list-unstyled d4m-cal-booking-details mypopupul">
                        <li>
                            ...
                            <div class="d4m-booking-status">
                                <?php 
								$rejed4mreason = $orderdetail[7];
                        ...
                        <li>
                            ...
                            <span class="span-scroll">: <?php  echo $general->d4mprice_format($orderdetail[2],$symbol_position,$decimal);
                               ?> </span>
                        </li>
						
						<?php   if($booking_duration_text != ""){ ?>
						<li class="<?php   if ($setting->get_option('d4mshow_time_duration') == 'N') {echo "force_hidden";} ?>">
                            <label><?php echo $label_language_values['duration'];?></label>
                            <span class="span-scroll">: <?php   echo $booking_duration_text; ?> </span>
                        </li>
						<?php    } ?>

                        <li><h6 class="d4m-customer-details-hr"><?php echo $label_language_values['customer'];?></h6>
                        </li>
                        <?php 
                        if($orderdetail[4]==0)
                        {
                            ...
                            <?php 
								...
								if($rejed4mreason != ""){
                                    ...
                                    <li>
                                        ...
                                        <span class="reason span-scroll">: <?php  echo $rejed4mreason;?></span>
                                    </li>
									<?php    
								}
								if($setting->get_option("d4mcompany_willwe_getin_status") == "Y") { ?>
                            <li>
                                <label><?php echo $label_language_values['contad4mstatus'];?></label>
                                <span class="notes span-scroll">: <?php  echo $temp['contad4mstatus'];?></span>
                            </li>
							<?php  
							}
                        }
                ...
                <div class="modal-footer">
                    <div class="cta-col12 d4m-footer-popup-btn text-center">
                        ...
                        if($orderdetail[6]=='C' || $orderdetail[6]=='R' || $orderdetail[6]=='CC' || $past_day == "yes"){
                            ?>
                            <span class="col-xs-4 np d4m-w-32">
                                <a data-id="<?php echo $_POST['orderid'];?>" class="btn btn-link confirm_book d4m-small-btn d4m-complete-appointment" title="<?php echo $label_language_values['complete_appointment'];?>"><i class="fa fa-thumbs-up fa-2x"></i><br /><?php echo $label_language_values['complete'];?></a>
                            </span>
                                <?php 
                        }
                        else{?>
								...
								<span class="col-xs-4 np d4m-w-32">
									<a data-id="<?php echo $_POST['orderid'];?>" class="btn btn-link d4m-small-btn d4m-confirm-appointment" title="<?php echo $label_language_values['confirm_appointment'];?>"><i class="fa fa-check fa-2x"></i><br /><?php echo $label_language_values['confirm'];?></a>
								</span>
								<span class="col-xs-4 np d4m-w-32">
									<a data-id="<?php echo $_POST['orderid'];?>" id="d4m-reject-appointment-cal-popup" class="btn btn-link d4m-small-btn book_rejectss" rel="popover" data-placement='top' title="<?php echo $label_language_values['rejed4mreason'];?>"><i class="fa fa-thumbs-o-down fa-2x"></i><br /><?php echo $label_language_values['reject'];?></a>
									...
									<div id="popover-reject-appointment-cal-popupss<?php  echo $_POST['orderid'];?>" style="display: none;">
										...
										<table class="form-horizontal" cellspacing="0">
											<tbody>
											<tr>
												<td><textarea class="form-control" id="reason_reject<?php  echo $_POST['orderid'];?>" name="" placeholder="<?php echo $label_language_values['appointment_rejed4mreason'];?>" required="required" ></textarea></td>
											</tr>
											<tr>
												<td>
													<button data-id="<?php echo $_POST['orderid'];?>" id="" value="Delete" class="btn btn-danger btn-sm rejed4mbookings" type="submit"><?php echo $label_language_values['reject'];?></button>
													<button id="d4m-close-reject-appointment-cal-popup" class="btn btn-default btn-sm" href="javascript:void(0)"><?php echo $label_language_values['cancel'];?></button>
												</td>
											</tr>
											</tbody>
										</table>
...
<?php 
	...
	$admin_company_name = $setting->get_option("d4mcompany_name");
	$setting_date_format = $setting->get_option("d4mdate_picker_date_format");
	$setting_time_format = $setting->get_option("d4mchoose_time_format");
	...
	$company_name = $setting->get_option("d4memail_sender_name");
	$company_email = $setting->get_option("d4memail_sender_address");
	$service_name = $clientdetail["title"];
	if ($admin_email == "") {
		$admin_email = $clientdetail["email"];
	}
	$price = $general->d4mprice_format($orderdetail[2], $symbol_position, $decimal); /* methods */
	$units = $label_language_values["none"];
	$methodname = $label_language_values["none"];
	$hh = $booking->get_methods_ofbookings($orderdetail[4]);
	$count_methods = mysqli_num_rows($hh);
	...
	if ($setting->get_option("d4mclient_email_notification_status") == "Y" && $clientemailtemplate[4] == "E") {
	...
	if ($setting->get_option("d4madmin_email_notification_status") == "Y" && $adminemailtemplate[4] == "E") {
	...
	if($staff_ids != ''){
		...
		if(sizeof((array)$staff_idss) > 0){
			foreach($staff_idss as $sid){
				...
				if($setting->get_option('d4mstaff_email_notification_status') == 'Y' && $staffemailtemplate[4]=='E' ){
				...
				/* TEXTLOCAL CODE */
				if($setting->get_option('d4msms_textlocal_status') == "Y"){
					if($setting->get_option('d4msms_textlocal_send_sms_to_staff_status') == "Y"){
				...
				/*PLIVO CODE*/
				if($setting->get_option('d4msms_plivo_status')=="Y"){
					if($setting->get_option('d4msms_plivo_send_sms_to_staff_status') == "Y"){
						if(isset($staff_phone) && !empty($staff_phone)){	
							$auth_id = $setting->get_option('d4msms_plivo_account_SID');
							$auth_token = $setting->get_option('d4msms_plivo_auth_token');
				...
				if($setting->get_option('d4msms_twilio_status') == "Y"){
					if($setting->get_option('d4msms_twilio_send_sms_to_staff_status') == "Y"){
						if(isset($staff_phone) && !empty($staff_phone)){
							$AccountSid = $setting->get_option('d4msms_twilio_account_SID');
							$AuthToken =  $setting->get_option('d4msms_twilio_auth_token'); 
				...
				if($setting->get_option('d4mnexmo_status') == "Y"){
					if($setting->get_option('d4msms_nexmo_send_sms_to_staff_status') == "Y"){
						if(isset($staff_phone) && !empty($staff_phone)){	
							$template = $objdashboard->gettemplate_sms("CO",'S');
							$phone = $staff_phone;				
							if($template[4] == "E") {
								...
								$d4mnexmo_text = str_replace($staff_searcharray,$staff_replacearray,$message);
	...
	if ($setting->get_option("d4msms_textlocal_status") == "Y") {
		if ($setting->get_option("d4msms_textlocal_send_sms_to_client_status") == "Y") {
		...
		if ($setting->get_option("d4msms_textlocal_send_sms_to_admin_status") == "Y") {
			$template = $objdashboard->gettemplate_sms("CO", "A");
			$phone = $setting->get_option("d4msms_textlocal_admin_phone");
			if ($template[4] == "E") {
				if ($template[2] == "") {
					$message = base64_decode($template[3]);
				} else {
	...
	if ($setting->get_option("d4msms_plivo_status") == "Y") {
		...
		$plivo_sender_number = $setting->get_option("d4msms_plivo_sender_number");
		$twilio_sender_number = $setting->get_option("d4msms_twilio_sender_number");
		if ($setting->get_option("d4msms_plivo_send_sms_to_client_status") == "Y") {
			$template = $objdashboard->gettemplate_sms("CO", "C");
			$phone = $client_phone;
			if ($template[4] == "E") {
				if ($template[2] == "") {
		...
		if ($setting->get_option("d4msms_plivo_send_sms_to_admin_status") == "Y") {
			$template = $objdashboard->gettemplate_sms("CO", "A");
	...
	if ($setting->get_option("d4msms_twilio_status") == "Y") {
		if ($setting->get_option("d4msms_twilio_send_sms_to_client_status") == "Y") {
			$template = $objdashboard->gettemplate_sms("CO", "C");
			$phone = $client_phone;
			if ($template[4] == "E") {
				if ($template[2] == "") {
		...
		if ($setting->get_option("d4msms_twilio_send_sms_to_admin_status") == "Y") {
			$template = $objdashboard->gettemplate_sms("CO", "A");
	...
	if ($setting->get_option("d4mnexmo_status") == "Y") {
		if ($setting->get_option("d4msms_nexmo_send_sms_to_client_status") == "Y") {
			$template = $objdashboard->gettemplate_sms("CO", "C");
			$phone = $client_phone;
			$phone = $client_phone;
			if ($template[4] == "E") {
		...
		if ($setting->get_option("d4msms_nexmo_send_sms_to_admin_status") == "Y") {
			$template = $objdashboard->gettemplate_sms("CO", "A");
			$phone = $setting->get_option("d4msms_nexmo_admin_phone_number");
			if ($template[4] == "E") {
				if ($template[2] == "") {
					$message = base64_decode($template[3]);
				} else {
				...
				$d4mnexmo_text = str_replace($searcharray, $replacearray, $message);
				$res = $nexmo_admin->send_nexmo_sms($phone, $d4mnexmo_text);
			}
		}
	}
	...
	$admin_company_name = $setting->get_option('d4mcompany_name');
	$setting_date_format = $setting->get_option('d4mdate_picker_date_format');
	$setting_time_format = $setting->get_option('d4mtime_format');
	$booking_date = str_replace($english_date_array,$selected_lang_label,date($setting_date_format,strtotime($clientdetail['booking_date_time'])));
	if($setting_time_format == 12){
		$booking_time = str_replace($english_date_array,$selected_lang_label,date("h:i A",strtotime($clientdetail['booking_date_time'])));
	} else {
...
elseif(isset($_POST['confirm_booking_cal'])){
  ...
  $price=$general->d4mprice_format($orderdetail[2],$symbol_position,$decimal);
	...
	if($orderdetail[4]==0){
		...
		$client_address=$temp['address'];
		$client_notes=$temp['notes'];
		$client_status=$temp['contad4mstatus'];		$client_city = $temp['city'];		$client_state = $temp['state'];		$client_zip	= $temp['zip'];
	} else {
		/*Registered user */
		$c  = $objdashboard->getguestclient($orderdetail[4]);
		$temppp= unserialize(base64_decode($c[5]));
  ...
  /*SMS SENDING CODE END*/
	...
	$d4mmax_advance_booking_time = $setting->get_option('d4mmax_advance_booking_time');
	$start_date = date("Y-m-d",$currDateTime_withTZ);
	$end_date = date("Y-m-d",strtotime("+".$d4mmax_advance_booking_time." months",$currDateTime_withTZ));
		...
		<li id="rec-noti-1" class="notificationli" data-orderid="<?php echo $b['order_id'];?>" data-booking_status="<?php     echo $b['booking_status']; ?>" style="background-color: <?php  echo $col;?>">
			<div class="list-inner">
				...
				if($b['client_id']==0)
				{
					...
					{
						$booking_stats='<span class="d4m-label bg-info br-2">'.$label_language_values['active'].'</span>';
					}
					elseif($b['booking_status']=='C')
					{
						$booking_stats='<span class="d4m-label bg-success br-2">'.$label_language_values['confirmed'].'</span>';
					}
					elseif($b['booking_status']=='R')
					{
						$booking_stats='<span class="d4m-label bg-danger br-2">'.$label_language_values['rejected'].'</span>';
					}
					elseif($b['booking_status']=='RS')
					{
						$booking_stats='<span class="d4m-label bg-primary br-2">'.$label_language_values['rescheduled'].'</span>';
					}
					elseif($b['booking_status']=='CC')
					{
						$booking_stats='<span class="d4m-label bg-warning br-2">'.$label_language_values['cancelled_by_client'].'</span>';
					}
					elseif($b['booking_status']=='CS')
					{

						$booking_stats='<span class="d4m-label bg-danger br-2">'.$label_language_values['cancelled_by_service_provider'].'</span>';
					}
					elseif($b['booking_status']=='CO')
					{
						$booking_stats='<span class="d4m-label bg-success br-2">'.$label_language_values['completed'].'</span>';
					}
					elseif($b['booking_status']=='GC')
					{
						$booking_stats='<span class="d4m-label bg-google br-2">Google Event</span>';
					}
					else
					{
						$booking_stats='<span class="d4m-label bg-default br-2">'.$label_language_values['mark_as_no_show'].'</span>';
					}
...
} elseif(isset($_POST['rejed4mbooking'])){
	$id = $_POST['order_id'];
	$reason = $_POST['rejed4mreason_book'];
	$gc_event_id = $_POST['gc_event_id'];
	$lastmodify = date('Y-m-d H:i:s');
	$objdashboard->rejed4mbookings($id,$reason,$lastmodify);
	$client_name = "";
	$orderdetail = $objdashboard->getclientorder($id);
  $clientdetail = $objdashboard->clientemailsender($id);
	$pid = $_POST['pid'];
	...
	if($gc_hook->gc_purchase_status() == 'exist'){
		echo $gc_hook->gc_cancel_rejed4mbooking_hook();
	}
	$admin_company_name = $setting->get_option('d4mcompany_name');
	$setting_date_format = $setting->get_option('d4mdate_picker_date_format');
	$setting_time_format = $setting->get_option('d4mtime_format');
			...
			<div class="modal-body">
				<div class="col-xs-12">
					<div class="form-group">
						<label class="cta-col2 d4m-w-50"><?php echo $label_language_values['amount']; ?>:</label>
						<div class="cta-col6">
				...
				<div class="col-xs-12">
					<div class="form-group">
						<label class="cta-col2 d4m-w-50"><?php echo $label_language_values['date_and_time']; ?>:</label>
						<div class="cta-col4 d4m-w-50">
							<?php 
						...
						<div class="cta-col6 d4m-w-50 float-right mytime_slots_booking">
							<?php 
							$t_zone_value = $setting->get_option('d4mtimezone');
							$server_timezone = date_default_timezone_get();
							if(isset($t_zone_value) && $t_zone_value!=''){
							...
							$seled4mtime=date('Y-m-d',strtotime($dates));
							$start_date = date($seled4mtime,$currDateTime_withTZ);
							$time_interval = $setting->get_option('d4mtime_interval');
							$time_slots_schedule_type = $setting->get_option('d4mtime_slots_schedule_type');
							$advance_bookingtime = $setting->get_option('d4mmin_advance_booking_time');
							$d4mservice_padding_time_before = $setting->get_option('d4mservice_padding_time_before');
							$d4mservice_padding_time_after = $setting->get_option('d4mservice_padding_time_after');
							$booking_padding_time = $setting->get_option('d4mbooking_padding_time');
							$time_schedule = $first_step->get_day_time_slot_by_provider_id($time_slots_schedule_type,$start_date,$time_interval,$advance_bookingtime,$d4mservice_padding_time_before,$d4mservice_padding_time_after,$timezonediff,$booking_padding_time,$staff_id);
							$allbreak_counter = 0;
							...
							<select class="selectpicker mydatepicker_appointment   form-control" id="myuser_reschedule_time" data-size="10" style="" >
								...
								if($time_schedule['off_day']!=true && isset($time_schedule['slots']) && sizeof((array)$time_schedule['slots'])>0 && $allbreak_counter != sizeof((array)$time_schedule['slots']) && $allofftime_counter != sizeof((array)$time_schedule['slots'])){
									foreach($time_schedule['slots']  as $slot) {
										...
										if($setting->get_option('d4mhide_faded_already_booked_time_slots')=='on' && in_array($complete_time_slot,$time_schedule['booked'])) {
											continue;
										}
										if( in_array($complete_time_slot,$time_schedule['booked']) && ($setting->get_option('d4mallow_multiple_booking_for_same_timeslot_status')!='Y') ) { ?>
											<?php 
											if($setting->get_option('d4mhide_faded_already_booked_time_slots')=="on"){
												?>
												<option value="<?php echo date("H:i",strtotime($slot));?>" <?php  if(date("H:i",strtotime($slot)) == $slot_timess){ echo "selected";}?> class="time-slot br-2 d4m-booked" >
													<?php 
													if($setting->get_option('d4mtime_format')==24){
										...
										<?php 
											...
											?>
											<option value="<?php echo date("H:i",strtotime($slot));?>" <?php  if(date("H:i",strtotime($slot)) == $slot_timess){ echo "selected";}?> class="time-slot br-2 <?php  if(in_array($complete_time_slot,$time_schedule['booked'])){ echo' d4m-booked';}else{ echo ' time_slotss'; }?>" <?php  if(in_array($complete_time_slot,$time_schedule['booked'])){echo ''; }else{ echo 'data-slot_date_to_display="'.date($getdateformate,strtotime($dates)).'" data-slot_date="'.$dates.'" data-slot_time="'.$slot_time.'"'; } ?>><?php if($setting->get_option('d4mtime_format')==24){echo date("H:i",strtotime($slot));}else{echo str_replace($english_date_array,$selected_lang_label,date("h:i A",strtotime($slot)));}?></option>
										<?php 
										} $slot_counter++;
									}
				...
				<div class="col-xs-12">
					<div class="form-group">
						<label class="cta-col2 d4m-w-50"><?php echo $label_language_values['notes']; ?>:</label>
						<div class="cta-col8">
