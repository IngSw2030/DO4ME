...
if (extension_loaded('zip')) {
	...
	{
		$getVersions = $settings->ext_get_contents('http://skymoonlabs.com/do4me/versioncheck.php?'.time());
		if ($getVersions != '')
		{
			/* If we managed to access that file, then lets break up those release versions into an array. */
			echo '<p>CURRENT VERSION: '.$settings->get_option('d4mversion').'</p>';
			echo '<p>Reading Current Releases List</p>';
			$versionList = explode("n", $getVersions);    
			foreach ($versionList as $aV)
			{
				if ( $aV > $settings->get_option('d4mversion')) 
				{
					echo '<p>New Update Found: v'.$aV.'</p>';
					$found = true;
					/* Download The File If We Do Not Have It */
					if ( !is_file(dirname(dirname(dirname(__FILE__))).'/d4m-updates/d4m-'.$aV.'.zip' )) 
					{
						echo '<p>Downloading New Update</p>';
						$newUpdate = $settings->url_get_contents($server_path.'d4m-updates/d4m-'.$aV.'.zip');
						if ( !is_dir( dirname(dirname(dirname(__FILE__))).'/d4m-updates/' ) ) 
							mkdir ( dirname(dirname(dirname(__FILE__))).'/d4m-updates/' );
						
						$dlHandler = fopen(dirname(dirname(dirname(__FILE__))).'/d4m-updates/d4m-'.$aV.'.zip', 'w');
					...
					{ 
						...
						$zipHandle = zip_open(dirname(dirname(dirname(__FILE__))).'/d4m-updates/d4m-'.$aV.'.zip');
			...
			if ($updated == true)
			{
				echo '<p class="success">&raquo; do4me Updated to v'.$aV.'</p>';
			}
