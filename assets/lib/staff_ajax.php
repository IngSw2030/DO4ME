...
$objservices = new do4me_services();
...
$bookings = new do4me_booking();
$bookings->conn = $conn;

/* ADDED START*/
$objdashboard = new do4me_dashboard();
$objdashboard->conn = $conn;

$general=new do4me_general();
$general->conn=$conn;

$staff_commision = new do4me_staff_commision();
...
$objdayweek_avail = new do4me_dayweek_avail();
$objdayweek_avail->conn = $conn;

$obj_offtime = new do4me_offtimes();
$obj_offtime->conn = $conn;

$objoffbreaks = new do4me_offbreaks();
$objoffbreaks->conn = $conn;

$offday=new do4me_provider_off_day();
$offday->conn = $conn;

$objpayment = new do4me_payments();
...
$getdateformat=$settings->get_option('d4mdate_picker_date_format');
$time_format = $settings->get_option('d4mtime_format');
$timess = "";
if($time_format == "24"){
	$timess = "H:i";
}
...
$getcurrency_symbol_position=$settings->get_option('d4mcurrency_symbol_position');
$getdateformate = $settings->get_option('d4mdate_picker_date_format');

$gettimeformat=$settings->get_option('d4mtime_format');
...
$admin_email = $settings->get_option('d4madmin_optional_email');
if($settings->get_option('d4mcompany_logo') != null && $settings->get_option('d4mcompany_logo') != ""){
	$business_logo= SITE_URL.'assets/images/services/'.$settings->get_option('d4mcompany_logo');
	$business_logo_alt= $settings->get_option('d4mcompany_name');
}else{
	$business_logo= '';
	$business_logo_alt= $settings->get_option('d4mcompany_name');
}
$company_city = $settings->get_option('d4mcompany_city');
$company_state = $settings->get_option('d4mcompany_state');
$company_zip = $settings->get_option('d4mcompany_zip_code');
$company_country = $settings->get_option('d4mcompany_country');
$company_phone = strlen($settings->get_option('d4mcompany_phone')) < 6 ? "" : $settings->get_option('d4mcompany_phone');
$company_email = $settings->get_option('d4mcompany_email');$company_address = $settings->get_option('d4mcompany_address'); 
/************ END ************/
...
if(isset($_POST['fullemail'])){
	if($_SESSION['d4museremail'] != trim(strip_tags(mysqli_real_escape_string($conn, $_POST['fullemail'])))){
...
elseif(isset($_POST['staff_update'])){
  ...
  $objadmin->APItestmode = $_POST['APItestmode'];
	...
	$new_service = implode(",",$_POST['d4mservice_staff']);
	
	$objadmin->d4mservice_staff = $new_service;
...
elseif(isset($_POST['dashboad_staff_update'])){
  $objadmin->id = $_POST['id'];
  $objadmin->fullname = $_POST['name'];
  $objadmin->email = $_POST['email'];
  $objadmin->description = $_POST['desc'];
...
elseif(isset($_POST['staff_detail']))
{
	...
	<div class="d4m-staff-details tab-content col-md-9 col-sm-8 col-lg-9 col-xs-12">
		<!-- right side common menu for staff -->
		<div class="d4m-staff-top-header">
			<span class="d4m-staff-member-name pull-left"><?php echo $staff_read['fullname'];?></span>
			
			<button id="d4m-delete-staff-member" class="pull-right btn btn-circle btn-danger" rel="popover" data-placement='left' title="<?php echo $label_language_values['delete_member'];?>"> <i class="fa fa-trash"></i></button>
					...
					<tbody>
						<tr>
							<td>
								...
								<button id="d4m-close-popover-delete-staff" class="btn btn-default" href="javascript:void(0)"><?php echo $label_language_values['cancel'];?></button>
							</td>
						</tr>
					</tbody>
				</table>
		...
		<hr id="hr" />
        <ul class="nav nav-tabs nav-justified d4m-staff-right-menu">
        ...
        <div class="tab-pane active"> <!-- first staff nmember -->
            <div class="container-fluid tab-content d4m-staff-right-details pall-15">
                <div class="tab-pane col-lg-12 col-md-12 col-sm-12 col-xs-12 active" id="member-details">
					<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
						<div class="d4m-clean-service-image-uploader">
						<?php  
						if($staff_read['image']==''){
						...
						?>
						<img data-imagename="" id="pppp<?php   echo $staff_read['id']; ?>staffimage" src="<?php echo $imagepath;?>" class="d4m-clean-staff-image br-100" height="100" width="100">
						<input data-us="pppp<?php   echo $staff_read['id']; ?>" class="hide d4m-upload-images" type="file" name="" id="d4m-upload-imagepppp<?php   echo $staff_read['id'];?>" data-id="<?php echo $staff_read['id'];?>" />
						<?php  
						if($staff_read['image']==''){
							?>
							<label for="d4m-upload-imagepppp<?php   echo $staff_read['id']; ?>" class="d4m-clean-staff-img-icon-label old_cam_ser<?php   echo $staff_read['id']; ?>">
								<i class="d4m-camera-icon-common br-100 fa fa-camera" id="pcls<?php   echo $staff_read['id']; ?>camera"></i>
						...
						<label for="d4m-upload-imagepppp<?php   echo $staff_read['id']; ?>" class="d4m-clean-staff-img-icon-label new_cam_ser ser_cam_btn<?php   echo $staff_read['id']; ?>" id="d4m-upload-imagepppp<?php   echo $staff_read['id']; ?>" style="display:none;">
							<i class="d4m-camera-icon-common br-100 fa fa-camera" id="pppp<?php   echo $staff_read['id']; ?>camera"></i>
							<i class="pull-left fa fa-plus-circle fa-2x" id="ctsc<?php   echo $staff_read['id']; ?>plus"></i>
						</label>
						<?php  
						if($staff_read['image']!==''){
							?>
							<a id="d4m-remove-staff-imagepppp<?php   echo $staff_read['id'];?>" data-pclsid="<?php echo $staff_read['id'];?>" data-staff_id="<?php echo $staff_read['id'];?>" class="delete_staff_image pull-left br-100 btn-danger bt-remove-staff-img btn-xs ser_new_del<?php   echo $staff_read['id'];?>" rel="popover" data-placement='left' title="<?php echo $label_language_values['remove_image'];?>"> <i class="fa fa-trash" title="<?php echo $label_language_values['remove_service_image'];?>"></i></a>
						<?php  
						}
						?>
					   <label><b class="error-service error_image" style="color:red;"></b></label>
						<div id="popover-d4m-remove-staff-imagepppp<?php   echo $staff_read['id'];?>" style="display: none;">
							<div class="arrow"></div>
							<table class="form-horizontal" cellspacing="0">
								<tbody>
								<tr>
									<td>
										...
										<a href="javascript:void(0)" id="d4m-close-popover-staff-image" class="btn btn-default btn-sm" href="javascript:void(0)"><?php echo $label_language_values['cancel'];?></a>
									</td>
								</tr>
								</tbody>
							</table>
					...
					<div id="d4m-image-upload-popuppppp<?php   echo $staff_read['id'];?>" class="d4m-image-upload-popup modal fade" tabindex="-1" role="dialog">
						<div class="vertical-alignment-helper">
							<div class="modal-dialog modal-md vertical-align-center">
								<div class="modal-content">
									<div class="modal-header">
										<div class="col-md-12 col-xs-12">
											<a data-staff_id="<?php echo $staff_read['id'];?>" data-us="pppp<?php   echo $staff_read['id'];?>" class="btn btn-success d4mupload_img_staff" data-imageinputid="d4m-upload-imagepppp<?php   echo $staff_read['id'];?>" data-id="<?php echo $staff_read['id']; ?>"><?php echo $label_language_values['crop_and_save'];?></a>
									...
									<div class="modal-body">
										<img id="d4m-preview-imgpppp<?php   echo $staff_read['id'];?>" style="width: 100%;"  />
									</div>
          ...
          <div class="col-lg-10 col-md-10 col-sm-10 col-xs-12 mt-10">
              <form id="staff_update_details">
								<div class="d4m-staff-common-table d4m-staff-common-table1">         
									
									<div class="form-group col-xs-12 col-md-12">
										<div class="col-xs-4 col-md-2"><label for="d4m-member-name"><?php echo $label_language_values['name'];?></label></div>
										<div class="col-xs-8 col-md-10"><input type="text" class="form-control" id="d4m-member-name" value="<?php echo $staff_read['fullname'];?>" name="u_member_name" /></div>
									</div>

									<div class="form-group col-xs-12 col-md-12">
										<div class="col-xs-4 col-md-2"><label for="d4m-member-name"><?php echo $label_language_values['email'];?></label></div>
										<div class="col-xs-8 col-md-10"><input type="text" class="form-control" id="d4m-member-email" value="<?php echo $staff_read['email'];?>" name="u_member_email" /></div>
									</div>
									
									<div class="form-group col-xs-12 col-md-12">
										<div class="col-xs-4 col-md-2"><label for="d4m-member-desc"><?php echo $label_language_values['description'];?></label></div>
										<div class="col-xs-8 col-md-10"><textarea class="form-control" id="d4m-member-desc" name="d4m-member-desc" ><?php echo $staff_read['description'];?></textarea></div>
									</div>

									<div class="form-group col-xs-12 col-md-12">
										<div class="col-xs-4 col-md-2"><label for="phone-number"><?php echo $label_language_values['phone'];?> </label></div>
										<div class="col-xs-8 col-md-10"><input type="tel" class="form-control" id="phone-number" name="phone-number" value="<?php echo $staff_read['phone'];?>" onkeyup="if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,'')" /></div>
									</div>
									
									<div class="form-group col-xs-12 col-md-12">
										<div class="col-xs-4 col-md-2"><label for="address"><?php echo "address"; ?></label></div>
										<div class="col-xs-8 col-md-10"><input type="text" class="form-control" name="d4m-member-address" id="d4m-member-address" placeholder="Member Street Address" value="<?php echo $staff_read['address']; ?>" />
										</div>
									</div>
										...
										<div class="col-xs-12">	
											<div class="col-xs-12 col-md-6 form-group npl npr">
												...
												<div class="col-xs-8">
													<input class="form-control value_city" placeholder="City" id="d4m-member-city" name="d4m-member-city" value="<?php echo $staff_read['city'];?>" type="text">
												</div>
											</div>
											...
											<div class="col-xs-12 col-md-6 form-group npl npr">
												...
												<div class="col-xs-8">
													<input class="form-control value_state" id="d4m-member-state" placeholder="State" name="d4m-member-state" type="text" value="<?php echo $staff_read['state'];?>">
												</div>
											</div>
										</div>
										...
										<div class="col-xs-12">
											<div class="col-xs-12 col-md-6 form-group npl npr">
												...
												<div class="col-xs-8">
													<input class="form-control value_zip" placeholder="Zip" id="d4m-member-zip" name="d4m-member-zip" type="text" value="<?php echo $staff_read['zip'];?>">
												</div>
											</div>
											...
											<div class="col-xs-12 col-md-6 form-group npl npr">
												...
												<div class="col-xs-8">
													<input class="form-control value_country" placeholder="Country" id="d4m-member-country" name="d4m-member-countrys" type="text" value="<?php echo $staff_read['country'];?>">
												</div>
											</div>
										</div>
                  ...
                  <div class="col-xs-12 col-md-6 pt-10 d4m-staff-common-table1">
                    <div class="col-xs-4"><label for="service"><?php echo $label_language_values['service'];?></label></div>
                    <div class="col-xs-8">
                      <select class="selectpicker mb-10" id="d4mservice_staff" multiple data-size="10" style="display: none;">
                        ...
                        ?>
								  ...
								  <div class="col-xs-12">
										<div class="col-xs-4 col-md-2"></div>
										<div class="col-xs-8 col-md-10">
										<a href="javascript:void(0);" id="update_staff_details" data-old_schedule_type="<?php echo $staff_read['schedule_type'];?>" class="btn btn-success d4m-btn-width mt-20" data-id="<?php echo $staff_read['id'];?>" type="submit"><?php echo $label_language_values['save'];?></a>
										</div>
                  </div>         
				 ...
				 <div class="tab-pane member-service-details" id="member-service-details">
                    <div class="panel panel-default">
						...
						<table id="d4m-staff-service-details-list" class="display responsive nowrap table table-striped table-bordered" cellspacing="0" width="100%">
							<thead>
								<th>#</th>
								...
								if(sizeof((array)$staff_service_details) > 0){
									foreach($staff_service_details as $arr_staff){
										...
										<tr>
											...
											<td><?php echo $general->d4mprice_format($get_booking_nettotal,$symbol_position,$decimal); ?></td>
										</tr>
									<?php   
									}
								}
				...
				<div class="tab-pane member-availability-details" id="member-availability-details">
					...
					<div class="panel panel-default">
						...
						<div class="tab-pane active"> <!-- first staff nmember -->
							<div class="container-fluid tab-content d4m-staff-right-details">
...
?>
									<form id="" method="POST">
										<div class="panel panel-default">
											<div class="col-sm-3 col-md-3 col-lg-3 col-xs-12 d4m-weeks-schedule-menu">
										...
										<div class="panel panel-default">
											<div class="panel-body">
												<?php if ($minweek == 1 && $maxweek == 1) { ?>
													<h4 class="d4m-right-header"><?php echo $label_language_values['this_week_time_scheduling']; ?></h4>
												<?php
        } else {
?>
													<h4 class="d4m-right-header"><?php echo $weekname[$i - 1]; ?><?php echo " " . $label_language_values['week_time_scheduling']; ?></h4>
												<?php
        } ?>
												<ul class="list-unstyled" id="d4m-staff-timing">
													<?php
        for ($j = 1;$j <= 7;$j++) {
...
?>
														<li class="active">
														<span
															class="col-sm-3 col-md-3 col-lg-3 col-xs-12 d4m-day-name"><?php echo $label_language_values[strtolower($objdayweek_avail->get_daynamebyid($j)) ]; ?></span>
													<span class="col-sm-2 col-md-2 col-lg-2 col-xs-12">
														<label class="cta-col2" for="d4m-monFirst<?php echo $i; ?><?php echo $j; ?>_<?php echo $getvalue[0]; ?>">
                ...
                echo $label_language_values['o_n'];
													...
													<span
														class="col-sm-7 col-md-7 col-lg-7 col-xs-12 d4m-staff-time-schedule">
...
?>
															<span class="d4m-staff-hours-to"> <?php echo $label_language_values['to']; ?> </span>
															<?php
            if ($time_format == 24) {
...
?>
									<!-- Start here -->
									<div class="col-sm-3 col-md-3 col-lg-3 col-xs-12 d4m-weeks-breaks-menu">
										<ul class="nav nav-pills nav-stacked">
											<?php
    if ($minweek == 1 && $maxweek == 5) {
        for ($i = $minweek;$i <= $maxweek;$i++) {
...
?>
									...
									<div class="col-sm-9 col-md-9 col-lg-9 col-xs-12 d4m-weeks-breaks-details">
										<div class="tab-content">
											<?php
    $breaks_weekname = array($label_language_values['first'], $label_language_values['second'], $label_language_values['third'], $label_language_values['fourth'], $label_language_values['fifth']);
    $breaks_weeknameid = array($label_language_values['first_week'], $label_language_values['second_week'], $label_language_values['third_week'], $label_language_values['fourth_week'], $label_language_values['fifth_week']);
...
?>
            ...
            echo "active";
													...
													<div class="panel panel-default">
														<div class="panel-body">
															<?php if ($minweek == 1 && $maxweek == 1) { ?>
																<h4 class="d4m-right-header"><?php echo $label_language_values['this_week_breaks']; ?> </h4>
															<?php
        } else { ?>
																<h4 class="d4m-right-header"><?php echo $breaks_weekname[$i - 1]; ?><?php echo $label_language_values['week_breaks']; ?> </h4>
															<?php
        } ?>
															<ul class="list-unstyled" id="d4m-staff-breaks">
																<?php
        for ($j = 1;$j <= 7;$j++) {
            $break_weekday = $j;
            $objdayweek_avail->week_id = $i;
...
?>
																	<li class="active">
																		<span class="col-sm-3 col-md-3 col-lg-3 col-xs-12 d4m-day-name"><?php echo $label_language_values[strtolower($objdayweek_avail->get_daynamebyid($j)) ]; ?></span>
																		<?php
            if ($getdatafrom_week_days[0] == 'Y' || $getdatafrom_week_days[0] == '') {
?>
																			<span class="col-sm-2 col-md-2 col-lg-2 col-xs-12">
																		<a class="btn btn-small btn-default d4m-small-br-btn disabled"><?php echo $label_language_values['closed']; ?></a>
																	</span>
																		<?php
            } else { ?>
																			<span class="col-sm-2 col-md-2 col-lg-2 col-xs-12">
...
?>
																		<span
																			class="col-sm-7 col-md-7 col-lg-7 col-xs-12 d4m-staff-breaks-schedule">
																		<ul class="list-unstyled" id="d4m-add-break-ul<?php echo $i; ?>_<?php echo $j; ?>">
																			<?php
																						...
																						<table class="form-horizontal" cellspacing="0">
																							...
																							<tr>
																								<td>
																									...
																									<button id="d4m-close-popover-delete-breaks" class="btn btn-default close_popup" href="javascript:void(0)"><?php echo $label_language_values['cancel']; ?></button>
																								</td>
																							</tr>
																							</tbody>
																						</table>
...
?>
						...
						<div class="panel panel-default">
						<div class="panel-body">
							<div class="d4m-member-offtime-inner">
								<h3>Off times</h3>
							</div>
																	<div class="d4m-staff-member-offtime-list-main mytablefor_offtimes cb col-md-12 col-xs-12">
																		<div class="table-responsive">
																			<table id="d4m-staff-member-offtime-list"
																					 class="d4m-staff-member-offtime-lists table table-striped table-bordered dt-responsive nowrap myadded_offtimes"
...
?>
    ...
    $displaydate = $offday->seled4mdate();
    ...
    for ($reihe = 1;$reihe <= 12;$reihe++) { /* 4 */
        ...
        echo '<td class="d4m-calendar-box col-lg-4 col-md-4 col-sm-6 col-xs-12 pull-left">';
        echo '<table align="center" class="table table-bordered table-striped monthtable">'; ?>
				<tbody class="ta-c">
					<div class="d4m-schedule-month-name pull-right">
						<div class="pull-left">
...
elseif(isset($_POST['assign_staff_booking'])){
	...
	if(sizeof((array)$staff_id) > 0){
		...
		$admin_company_name = $settings->get_option('d4mcompany_name');
		$setting_date_format = $settings->get_option('d4mdate_picker_date_format');
		$setting_time_format = $settings->get_option('d4mtime_format');
		...
		$company_name = $settings->get_option('d4memail_sender_name');
		$company_email = $settings->get_option('d4memail_sender_address');
		$service_name = $clientdetail['title'];
		if($admin_email == ""){
			$admin_email = $clientdetail['email'];	
		}
		
		$price=$general->d4mprice_format($orderdetail[2],$symbol_position,$decimal);
		...
		if($orderdetail[4]==0)
		{
			...
			$client_status=$temp['contad4mstatus'];				
		...
		foreach($staff_id as $sid){
			...
			$searcharray = array('{{service_name}}','{{booking_date}}','{{business_logo}}','{{business_logo_alt}}','{{client_name}}','{{methodname}}','{{units}}','{{addons}}','{{client_email}}','{{phone}}','{{payment_method}}','{{vaccum_cleaner_status}}','{{parking_status}}','{{notes}}','{{contad4mstatus}}','{{address}}','{{price}}','{{admin_name}}','{{firstname}}','{{lastname}}','{{app_remain_time}}','{{rejed4mstatus}}','{{company_name}}','{{booking_time}}','{{client_city}}','{{client_state}}','{{client_zip}}','{{company_city}}','{{company_state}}','{{company_zip}}','{{company_country}}','{{company_phone}}','{{company_email}}','{{company_address}}','{{admin_name}}','{{staff_name}}','{{staff_email}}');
			...
			if($settings->get_option('d4mstaff_email_notification_status') == 'Y' && $staffemailtemplate[4]=='E' ){
				$client_email_body = str_replace($searcharray,$replacearray,$stafftemplate).$new_div;
				if($settings->get_option('d4msmtp_hostname') != '' && $settings->get_option('d4memail_sender_name') != '' && $settings->get_option('d4memail_sender_address') != '' && $settings->get_option('d4msmtp_username') != '' && $settings->get_option('d4msmtp_password') != '' && $settings->get_option('d4msmtp_port') != ''){
					$mail->IsSMTP();
				}else{
					$mail->IsMail();
				}
			...
			/* TEXTLOCAL CODE */
			if($settings->get_option('d4msms_textlocal_status') == "Y")
			{
				if($settings->get_option('d4msms_textlocal_send_sms_to_staff_status') == "Y"){
			...
			/*PLIVO CODE*/
			if($settings->get_option('d4msms_plivo_status')=="Y"){							
				if($settings->get_option('d4msms_plivo_send_sms_to_staff_status') == "Y"){
					if(isset($staff_phone) && !empty($staff_phone))
					{ 
						...
						if($template[4] == "E"){
								...
								$params = array(
										'src' => $settings->get_option('d4msms_plivo_sender_number'),
										'dst' => $phone,
			...
			if($settings->get_option('d4msms_twilio_status') == "Y"){							
				if($settings->get_option('d4msms_twilio_send_sms_to_staff_status') == "Y"){
					if(isset($staff_phone) && !empty($staff_phone))
					{	
			...
			if($settings->get_option('d4mnexmo_status') == "Y"){				
				if($settings->get_option('d4msms_nexmo_send_sms_to_staff_status') == "Y"){
					if(isset($staff_phone) && !empty($staff_phone))
					{	
						$template = $objdashboard->gettemplate_sms("RS",'S');
						$phone = $staff_phone;				
						if($template[4] == "E") {
							...
							$d4mnexmo_text = str_replace($searcharray,$replacearray,$message);
							$res=$nexmo_client->send_nexmo_sms($phone,$d4mnexmo_text);
						}
					}
				}
			}
			...
			if(mysqli_num_rows($all_bookings) > 0){
				...
				?>
					<tr>
						...
						<td><?php echo  $general->d4mprice_format($net_total,$symbol_position,$decimal); ?></td>
						<td><?php echo $general->d4mprice_format($get_booking_nettotal,$symbol_position,$decimal); ?></td>
			...
			if(mysqli_num_rows($all_bookings) >0){
				...
				while($row = mysqli_fetch_array($all_bookings)){
					...
					<tr>
						...
						<td><?php echo  $general->d4mprice_format($row['amt_payable'],$symbol_position,$decimal);?></td>
						<td><?php echo  $general->d4mprice_format($row['advance_paid'],$symbol_position,$decimal);?></td>
						<td><?php echo  $general->d4mprice_format($row['net_total'],$symbol_position,$decimal);?></td>
					</tr>
					<?php  
					$i++;
				}
...
if(isset($_POST['action']) && filter_var($_POST['action'], FILTER_SANITIZE_STRING)=='money_transfer_request'){
  $objpayment->email_id=$_POST['email_id'];
  $objpayment->staffid=$_POST['staffid'];
  $objpayment->transfer_amount_value=$_POST['transfer_amount_value'];
  $result=$objpayment->staff_payment_request(); 
}

/* Verify OTP action Jay W */
if(isset($_POST['Verify_otp'])){
  $objadminprofile->otp = $_POST['otp_input'];
  $objadminprofile->phone1 = $_POST['phone'];
  $simple_phone_number = $_POST['phone'];
  $company_contry_code = $settings->get_option("d4mcompany_country_code");
  $contry_code_array = explode(",",$company_contry_code);
  $objadminprofile->phone = $contry_code_array[0].$simple_phone_number;
  $result = $objadminprofile->verify_otp();
  if($result[0] == ""){
    echo $result = 0;
  }else{
    echo $result = 1;
...
/* Verify OTP action end Jay W */

/* Send OTP action Jay W */
if(isset($_POST['sendotp'])){
  $simple_phone_number = $_POST['phoneno'];
  $company_contry_code = $settings->get_option("d4mcompany_country_code");
  $contry_code_array = explode(",",$company_contry_code);
  $objadminprofile->phoneno = $contry_code_array[0].$_POST['phoneno'];
  $otp = rand(100000,999999);
  $objadminprofile->otp = $otp;

  $checkotp = $objadminprofile->check_otp_for_phone();
  if($checkotp[0] != ''){
    $result = $objadminprofile->update_otp();
  }else{
    $result = $objadminprofile->insert_otp();
  }
  $staff_phone = $contry_code_array[0].$_POST['phoneno'];
  ...
  if($settings->get_option("d4msms_textlocal_status") == "Y"){
    ...
    $message = "Your One time Password for Registration is"."".$otp.".Please Verify Once For Registration.";

    $data = "username=".$textlocal_username."&hash=".$textlocal_hash_id."&message=".$message."&numbers=".$staff_phone."&test=0";
  ...
  if($settings->get_option("d4msms_plivo_status")=="Y"){
    ...
    $obj_plivo = new Plivo\RestAPI($settings->get_option("d4msms_plivo_account_SID"), $settings->get_option("d4msms_plivo_auth_token"), "", "");
 ...
 if($settings->get_option("d4mnexmo_status") == "Y"){
    $obj_nexmo = new do4me_d4mnexmo();
    $obj_nexmo->d4mnexmo_api_key = $settings->get_option("d4mnexmo_api_key");
    $obj_nexmo->d4mnexmo_api_secret = $settings->get_option("d4mnexmo_api_secret");
    $obj_nexmo->d4mnexmo_from = $settings->get_option("d4mnexmo_from");

    $message = "Your One time Password for Registration is "."".$otp.".Please Verify Once For Registration.";
    $res=$obj_nexmo->send_nexmo_sms($phone,$message);
  }

  echo json_encode($simple_phone_number);

}
/* Send OTP action end Jay W */ 
?>