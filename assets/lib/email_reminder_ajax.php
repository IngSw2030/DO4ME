...
$objocinfo = new do4me_order_client_info();
$objocinfo->conn = $conn;

$objfreqdis = new do4me_frequently_discount();
...
$company_city = $setting->get_option('d4mcompany_city');
$company_state = $setting->get_option('d4mcompany_state');
$company_zip = $setting->get_option('d4mcompany_zip_code');
$company_country = $setting->get_option('d4mcompany_country');
...
while($e_reminder = mysqli_fetch_array($book_details)){
	...
	$email_reminder_time=$setting->get_option('d4memail_appointment_reminder_buffer');
	...
	if($email_reminder_time >= $time_in_min && $binfo[12]!=1){
		...
		if($setting->get_option('d4msms_plivo_status')=="Y"){
			...
			if($setting->get_option('d4msms_plivo_send_sms_to_admin_status') == "Y"){
				...
				$admin_phone = $setting->get_option('d4msms_plivo_admin_phone_number');
				$template = $objdashboard->gettemplate_sms("RM",'A');
		...
		if($setting->get_option('d4msms_twilio_status') == "Y"){
			...
			if($setting->get_option('d4msms_twilio_send_sms_to_admin_status') == "Y"){
				...
				$admin_phone = $setting->get_option('d4msms_twilio_admin_phone_number');
				$template = $objdashboard->gettemplate_sms("RM",'A');
				if($template[4] == "E") {
					if($template[2] == ""){
						$message = base64_decode($template[3]);
...
$appointment_auto_confirm=$setting->get_option("d4mappointment_auto_confirm_status");
$booking_status="A";
...
if($setting->get_option('d4mstripe_payment_form_status') == "on" && $secret_key != ""){
