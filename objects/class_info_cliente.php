...
class do4me_order_client_info{
	...
	public $service_id;
	public $conn;
	public $table_name="d4morder_client_info";
	public $tablename3="d4musers";
	public $tablename_rec="d4mrecurring_status";
	...
	*/
	...
	public function readone_for_email(){
		$query="select `email`,`fullname` from `d4madmin_info`";
		$result=mysqli_query($this->conn,$query);
	...
	public function get_clone_order_id(){
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			$qq="select `order_id` from `d4mbookings` where `order_id`='".$b_order_id."'";
